...
this.jQun = function(Function, create, getProperties, setPrototypeOf){
	return function jQun(constructor, _SuperClass){
		...
		setPrototypeOf(
			...
			prototype ?
				(
					_SuperClass ? prototype.constructor : this.constructor.getOwnPrototype()
				) :
	...
	return function every(obj, fn, _this){
		...
		for(
			...
			if(
				fn.apply(
					_this,
					isNumber ? [name - 0] : [obj[name], name, obj]
				)
			){
				continue;
			}
...
this.defineProperties = function(set, defineProperty, empty, toObject){
	return function defineProperties(obj, props, _descriptor){
		...
		///	<param name="props" type="Object, Array">包含一个或多个属性的键值对或者是带有name、value的对象数组。</param>
...
// 原型链属性的定义
new function(Function, prototype, forEach, defineProperties, getPrototypeOf, definePrototype){
"use strict";
...
this.Interface = function(Class, isInstanceOf, getConstructor){
	...
	function(propertyNames, constructor, args){
		...
		var Super = this.getSuperClass.call(
...
this.UnopenedNamespace = function(CSS_REGEXP, createElement, getOnload){
	function UnopenedNamespace(filename, _onopen){
		///	<summary>
		///	未开辟的命名空间，适用于模块化开发。
		///	</summary>
...
this.AnonymousSpace = function(toArray){
	function AnonymousSpace(callback, _refs){
		///	<summary>
		///	开辟一个匿名空间，匿名空间的目的是为了管理引用项与回调。
		///	</summary>
		///	<param name="callback" type="Function">当引用加载项全部加载完成时所执行的回调函数。</param>
		///	<param name="_refs" type="*">引用项：arguments参数的第2至N个参数都为引用项。</param>
		if(
			currentlyCallback
		){
			throw "同一脚本代码环境中，不允许连续实例化多个AnonymousSpace！\n即一个AnonymousSpace将对应一个UnopenedNamespace。";
			return;
		}

		// 页面onload的时候会创建此实例，就可以将信息记录下来，然后马上就会运行script的Onload，就可以获取到信息。
		currentlyCallback = function(){
	...
	return AnonymousSpace;
}(
	jQun.toArray
);

this.Namespace = function(UnopenedNamespace, SEPARATOR_REGEXP, window){
	function Namespace(_members){
		///	<summary>
		///	开辟一个命名空间，并可赋予初始成员或将指定的命名空间字符串解析成对应的命名空间。
		///	</summary>
		///	<param name="members" type="Object, String">成员键值对或者命名空间字符串。</param>
		if(
			!_members
		){
			return;
		...
		if(
			typeof _members === "string"
		){
			var current = window;

			(
				_members.match(SEPARATOR_REGEXP) || []
			)
			.forEach(
				function(name){
					var ns = current[name];
		...
		members : function(members){
			...
			forEach(
				...
				function(member, name){
					var existedMember = currentNS[name];

					if(
						existedMember instanceof constructor
					){
						existedMember.members(member);
						return;
					}
...
this.BrowserCore = function(){
	return new Enum(
		["Unknow", "AppleWebKit", "Gecko", "Trident"]
	);
}();

this.BrowserAgent = function(){
	return new Enum(
		["Unknow", "Chrome", "Firefox", "MSIE", "Safari", "Opera", "MicroMessenger", "MQQBrowser"]
	);
}();

this.Browser = function(OS, BrowserCore, BrowserAgent, userAgent, parseInt){
	function Browser(){
		///	<summary>
		///	浏览器信息。
		///	</summary>
		...
		[
			...
			this[i].forEach(
				...
				{ agent : BrowserAgent, core : BrowserCore, os : OS }
			);
		...
		if(
			...
			if(
				...
				info.agent = BrowserAgent.Safari;
			}
		}
		else if(
			core === BrowserCore.Trident
		){
			info.agent = BrowserAgent[info.isMobile ? "WindowsPhone" : "MSIE"];
			info.isIE9 = info.agent === BrowserAgent.MSIE && parseInt(info.version) === 9;
		}

		info.agentName = BrowserAgent.getNameByValue(info.agent);

		this.assign(info);
	};
	Browser = new StaticClass(Browser, {
		// 浏览器代理商
		agent : BrowserAgent.Unknow,
		// 浏览器名称
		agentName : BrowserAgent.getNameByValue(BrowserAgent.Unknow),
		// 浏览器核心
		core : BrowserCore.Unknow,
		// 浏览器核心版本
		coreVersion : "0",
		isIE9 : false,
		// 操作系统
	...
	Browser.static({
		getMobileOSVersion : function(){
			...
			if(
				...
				var result = userAgent.match(this.os === OS.Android ? /Android ([\d\.]+)/ : /OS ([\d\_]+)/);
...
this.Verification = function(RegExp){
...
this.EventTargetList = function(toArray){
	...
	EventTargetList.props({
		...
		listen : function(events, _capture, _priority, _useWeakReference){
			///	<summary>
			///	向集合中所有目标注册事件侦听器。
			///	</summary>
		...
		loaded : function(handler){
			...
			var docTarget = new EventTargetList([document]),
				...
				loaded = function(){
					...
					docTarget.unlisten({ DOMContentLoaded : loaded });
				};
...
this.ElementList = function(NodeList, Node, ID, selectorText, document, map){
	...
	ElementList.override({
		createList : function(_selector, _parent){
			...
			return new ElementList(_selector, _parent);
		}
	});
	...
	ElementList.props({
		...
		delegate : function(selector, events, _capture, _priority, _useWeakReference){
			///	<summary>
			///	向集合中所有目标注册事件侦听器。
			///	</summary>
			///	<param name="selector" type="String, Node">指定的元素选择器。</param>
			///	<param name="events" type="Object">事件侦听器键值对。</param>
			///	<param name="_capture" type="Boolean">侦听器是否运行于捕获阶段。</param>
			///	<param name="_priority" type="Number">优先级，数字越大，优先级越高。</param>
			///	<param name="_useWeakReference" type="Boolean">是否是属于强引用。</param>
			this.listen(
				map(
					events,
					function(fn){
						return function(e, targetList){
							var list = targetList.btw(selector, this);
							...
							fn.apply(list[0], fn.length === 2 ? [e, targetList] : [e]);
						};
					},
					this
				),
		...
		query : function(_selector){
			...
			var constructor = ElementList, list = this.createList();
...
this.HTMLElementList = function(ElementList, getCSSNumberValue, getComputedStyle, addProperty){
	...
	HTMLElementList.override({
		createList : function(_selector, _parent){
			...
			///	<param name="_selector" type="String, Element">选择器或dom元素。</param>
			///	<param name="_parent" type="Element">指定查询的父元素。</param>
			return new HTMLElementList(_selector, _parent);
		}
	});
...
this.ReposcursorEvent = function(Browser, BrowserAgent, reposcursorEvent, selection){
...
　　						shakeEvent.trigger(
	...
	ShakeEvent = new Class(ShakeEvent);
...
// 与重写有关
(function(EventTargetList, Event, HTMLElementList, HTML, Browser, BrowserAgent, document, dispatch){
	...
	function OverrideDispatch(){
		...
		EventTargetList.override({
			dispatch : function(event, _attrs){
				...
				*/
				if(
					Browser.agent !== BrowserAgent.Chrome
				){
...
// 与ajax相关的类
(function(JSON, Browser, BrowserAgent, XMLHttpRequest, forEach, set){
...
this.RequestConnection = function(FormData, RequestMethod, ResponseType, isIE9, toCheck, getEncodedParams){
	function RequestConnection(name, url, _Interface, _method, _responseType, _key){
		...
		///	<param name="_method" type="RequestMethod">发送数据的方式。</param>
		///	<param name="_responseType" type="ResponseType">返回的数据格式。</param>
	...
	RequestConnection.props({
		...
		create : function(urlQueryString, _formData, _load, _error, _beforeSend){
			...
			request.open(
				RequestMethod.getNameByValue(
					_formData ? RequestMethod.Post : this.method,
					true
				),
			...
			try{
				...
				request.responseType = ResponseType.getNameByValue(responseType, true);
			}
			catch(e){}
			...
			request.addEventListener(
				"load",
				function(){
					var response = this[isIE9 ? "responseText" : "response"];
		...
		open : function(params, _load, _error, _beforeSend){
			...
			if(
				...
				var formData = new (isIE9 ? Storage : FormData)(), method = isIE9 ? "set" : "append";

				forEach(
					params,
					function(value, name){
						formData["method"](name, value);
					}
				);
...
this.Ajax = function(RequestConnection, Storage, Blob, ResponseType, console, location){
	function Ajax(){
		///	<summary>
		///	ajax异步类。
		///	</summary>
		...
		open : function(name, data, _load, _error, _beforeSend, _progress){
			...
			if(
				...
				if(
					...
					_load(requestConnection.responseType === ResponseType.Json ? {} : "");
				}
...
// 与socket相关的类
(function(EventTargetList, Event, TYPE, window, listen){
...
this.Port = function(Socket, WINDOW_OBJECT_STRING, undefined, messageEvent, dispatch, set, toArray){
	...
	Port.override({
		listen : function(events, _capture, _priority, _useWeakReference){
			...
			return listen.call(this, events, _capture, _priority, _useWeakReference);
		},
		dispatch : function(event, _attrs){
			///	<summary>
			///	给所有目标分配事件。
...
this.TransferPort = function(CrossWindowPort, Socket){
	...
	"__jQun.Port__",
	window,
	this.EventTargetList.prototype.listen
));
