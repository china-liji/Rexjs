...
this.ECMAScriptVariableCollections = function(generate){
	/**
	 * 变量名收集器集合
	 * @param {VariableIndex} index - 变量名索引
	 */
	function ECMAScriptVariableCollections(index){
...
this.ECMAScriptStatements = function(ECMAScriptStatement, ECMAScriptVariableCollections, VariableCollection, VariableIndex, extractTo){
	/**
	 * ECMAScript 语句块
	 * @param {ECMAScriptVariableCollections} _prevCollections - 上一个变量名收集器
	 */
	function ECMAScriptStatements(_prevCollections){
		Statements.call(this);

		// 初始化变量名集合
		this.initCollections(_prevCollections);
	};
	...
	ECMAScriptStatements.props({
		 ...
		 * @param {ECMAScriptVariableCollections} _prevCollections - 上一个变量名收集器
		 */
		initCollections: function(_prevCollections){
			var collections = new ECMAScriptVariableCollections(
				_prevCollections ? _prevCollections.index : new VariableIndex()
			);

			collections.blacklist = new VariableCollection();
			collections.const = new VariableCollection();
			collections.declaration = new VariableCollection();
			collections.rex = new VariableCollection();
...
this.VariableTag = function(IdentifierTag){
	...
	VariableTag.props({
		 ...
		 * @param {Context} context - 标签上下文
		 */
		error: function(parser, context){
			parser.error(context, "Assignment to constant variable.");
		},
		order: 201
	});
...
this.VariableDeclarationTag = function(VariableTag, visitor){
	...
	VariableDeclarationTag.props({
		 ...
		 */
		error: function(parser, context){
			parser.error(context, "Identifier \"" + context.content + "\" has already been declared");
		},
		/**
		 * 标签访问器
		 * @param {SyntaxParser} parser - 语法解析器
...
this.UnaryStatement = function(){
	...
	UnaryStatement.props({
		...
		try: function(parser, context){
			...
			if(
				this.target.expression.context.tag.isSeparator(context.tag)
			){
				// 跳出语句并设置 operand
				this.out().operand = this.expression;
			}
...
this.UnaryTag = function(UnaryExpression, UnaryStatement, ExpressionSeparatorTag){
	...
	UnaryTag.props({
		...
		/**
		 * 验证所提供的标签是否为表达式分隔符标签
		 * @param {SyntaxTag} tag - 所需验证的标签
		 */
		isSeparator: function(tag){
			return tag instanceof ExpressionSeparatorTag;
		},
		/**
		 * 获取此标签接下来所需匹配的标签列表
		 * @param {TagsMap} tagsMap - 标签集合映射
...
// 一元赋值标签
void function(AssignableExpression, UnaryStatement, VariableTag){
	...
	function(parser, statement, context, method){
		...
		if(
			...
			switch(
				...
				// 如果不是变量标签
				case tag instanceof VariableTag:
					break;
		...
		// 报错
		parser.error(statement.target.expression.context, "Invalid left-hand side expression in prefix operation", true);
	}
);
...
// 二元标签基类
void function(ExpressionSeparatorTag){
...
this.BinaryStatement = function(){
	...
	BinaryStatement.props({
		...
		try: function(parser, context){
			// 如果是表达式分隔符标签
			if(
				context.tag instanceof ExpressionSeparatorTag
			){
				// 跳出语句并添加表达式
				this.out().add(this.expression);
			}
...
this.AssignmentTag = function(AssignableExpression, BinaryExpression, VariableTag, VariableDeclarationTag, visitor){
	/**
	 * 二元赋值运算符标签
	 * @param {Number} _type - 标签类型
	 */
	...
	AssignmentTag.props({
		 ...
		 */
		visitor: function(parser, context, statement, statements){
			var ctx, expression = statement.expression;
			...
			switch(
				...
				case expression instanceof BinaryExpression:
					...
					if(
						...
						ctx = expression[expression.length - 1].context;
						break;
					}
...
this.NewTag = function(OpenCallTag, isSeparator){
	/**
	 * new 关键字标签
	 * @param {Number} _type - 标签类型
	 */
	...
	NewTag.props({
		 ...
		 */
		isSeparator: function(tag){
			return tag instanceof OpenCallTag || isSeparator.call(this, tag);
		},
		regexp: /new/,
		/**
		 * 获取此标签接下来所需匹配的标签列表
...
this.ArrayItemSparatorTag = function(CommaTag, ArrayStatement){
	/**
	 * 数组项分隔符标签
	 * @param {Number} _type - 标签类型
	 */
...
this.BlockBodyStatements = function(ECMAScriptStatements, BlockBodyStatement){
	 ...
	 * @param {ECMAScriptVariableCollections} _prevCollections - 上一个变量名收集器
	 */
	function BlockBodyStatements(_prevCollections){
		ECMAScriptStatements.call(this, _prevCollections);
	};
...
this.OpenBlockTag = function(OpenBraceTag, BlockExpression, BlockBodyStatements){
	/**
	 * 起始语句块标签
	 * @param {Number} _type - 标签类型
	 */
	...
	OpenBlockTag.props({
		...
		visitor: function(parser, context, statement, statements){
			...
			// 设置当前语句块
			(
				parser.statements = new BlockBodyStatements(statements.collections)
			)
			.target = statements;
		}
	});
...
// 函数表达式相关
void function(RestArgumentExpression, VariableDeclarationTag, BasicAssignmentTag, config, argumentSeparatorTag, closeArgumentsTag, closeFunctionBodyTag, closeFunctionDeclarationBodyTag){

this.FunctionExpression = function(insertDefaults){
	/**
	 * 函数表达式
	 * @param {Expression} head - 函数头部表达式
	 */
	function FunctionExpression(head){
		Expression.call(this, head.context);
	...
	FunctionExpression.props({
		...
		extractTo: function(contentBuilder){
			...
			// 提取函数参数
			this.arguments.extractTo(contentBuilder, anotherBuilder);

			// 如果没有默认或省略参数
			if(
				anotherBuilder.complete().length === 0
			){
				// 直接提取函数主体
			...
			// 插入默认参数
			insertDefaults(contentBuilder, this.body, anotherBuilder.result);
		},
		isDeclaration: false
	});
	...
	// insertDefaults
	function(contentBuilder, body, result){
		var inner = body.inner, builder = new ContentBuilder();
		
		// 追加起始大括号
		contentBuilder.appendContext(body.open);
		// 提取第一个表达式至临时生成器
		...
		// 追加结束大括号
		contentBuilder.appendContext(body.close);
	}
);
...
this.ArgumentExpression = function(AssignableExpression){
	/**
	 * 参数表达式
	 * @param {Context} context - 语法标签上下文
	 */
	...
	ArgumentExpression = new Rexjs(ArgumentExpression, AssignableExpression);
...
this.RestArgumentExpression = RestArgumentExpression = function(ArgumentExpression){
	/**
	 * 省略参数表达式
	 * @param {Context} context - 语法标签上下文
	...
	RestArgumentExpression.props({
		...
		extractTo: function(contentBuilder, anotherBuilder){
			...
			if(
				...
				// 将默认参数名追加至临时生成器
				anotherBuilder.appendContext(this.context);
				// 将赋值运算追加至临时生成器
...
this.ArgumentsStatement = function(throwError){
	/**
	 * 参数语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
	function ArgumentsStatement(statements){
		ECMAScriptStatement.call(this, statements);
	};
	ArgumentsStatement = new Rexjs(ArgumentsStatement, ECMAScriptStatement);

	ArgumentsStatement.props({
		/**
		 * 捕获处理异常
		 * @param {SyntaxParser} parser - 语法解析器
		 * @param {Context} context - 语法标签上下文
		 */
			...
			if(
				...
				// 抛出错误
				throwError(parser, context, this.expression.inner);
				return null;
			}

			// 跳出语句并设置 arguments
			this.out().arguments = this.expression;
			return closeArgumentsTag;
		}
	});

	return ArgumentsStatement;
}(
	// throwError
	function(parser, context, inner){
		var description = "";
		...
		if(
			...
			// 如果最后一个表达式是省略参数，则说明逗号是由于省略参数而导致的异常
			if(
				inner[inner.length - 1] instanceof RestArgumentExpression
			){
				// 修改错误描述
				description = "Rest parameter must be last formal parameter";
			}
		}

		// 报错
		parser.error(context, description);
	}
);

this.ArgumentNameStatement = function(){
	/**
	 * 参数名语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
	function ArgumentNameStatement(statements){
		ECMAScriptStatement.call(this, statements);
	};
	ArgumentNameStatement = new Rexjs(ArgumentNameStatement, ECMAScriptStatement);

	ArgumentNameStatement.props({
		/**
		 * 捕获处理异常
		 * @param {SyntaxParser} parser - 语法解析器
		 * @param {Context} context - 语法标签上下文
		 */
			...
			// 如果是逗号，返回 argumentSeparatorTag 标签
			return context.content === "," ? argumentSeparatorTag : null;
		},
		/**
		 * 尝试处理异常
		 * @param {SyntaxParser} parser - 语法解析器
...
this.FunctionBodyStatement = function(BlockBodyStatement){
	/**
	 * 函数主体语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
	function FunctionBodyStatement(statements){
		BlockBodyStatement.call(this, statements);
	};
	FunctionBodyStatement = new Rexjs(FunctionBodyStatement, BlockBodyStatement);

	FunctionBodyStatement.props({
		/**
		 * 捕获处理异常
		 * @param {SyntaxParser} parser - 语法解析器
		 * @param {Context} context - 语法标签上下文
		 */
			...
			var statements = this.statements, targetStatements = statements.target, functionExpression = targetStatements.statement.expression;

			// 恢复语句块
			parser.statements = targetStatements;
			// 设置函数主体内容
			functionExpression.body.inner = statements;

			// 返回结束语句块标签
			return functionExpression.isDeclaration ? closeFunctionDeclarationBodyTag : closeFunctionBodyTag;
		}
	});
...
this.FunctionBodyStatements = function(BlockBodyStatements, FunctionBodyStatement){
	/**
	 * 函数主体语句块
	 */
	function FunctionBodyStatements(){
		BlockBodyStatements.call(this);
	};
	...
	FunctionBodyStatements.props({
		 ...
		 */
		initStatement: function(){
			return new FunctionBodyStatement(this);
		}
	});
...
this.FunctionTag = function(FunctionExpression){
	...
	FunctionTag.props({
		...
		visitor: function(parser, context, statement, statements){
			// 设置当前表达式
			statement.expression = new FunctionExpression(
...
this.FunctionDeclarationTag = function(FunctionTag, visitor){
	/**
	 * 函数声明标签
	 * @param {Number} _type - 标签类型
	 */
	...
	FunctionDeclarationTag.props({
		...
		visitor: function(parser, context, statement, statements){
			...
			// 设置 isDeclaration，表明是声明
			statement.expression.isDeclaration = true;
		}
	});
	...
	this.FunctionTag,
	this.FunctionTag.prototype.visitor
);

this.FunctionNameTag = function(){
	/**
	 * 函数名标签
	 * @param {Number} _type - 标签类型
	 */
	...
	FunctionNameTag.props({
		...
		visitor: function(parser, context, statement, statements){
			...
			// 设置函数表达式的名称
			expression.name = new Expression(context);
		}
	});
...
this.OpenArgumentsTag = function(OpenParenTag, ArgumentsExpression, ArgumentsStatement){
	/**
	 * 起始函数参数标签
	 * @param {Number} _type - 标签类型
	 */
	...
	OpenArgumentsTag.props({
		...
		visitor: function(parser, context, statement, statements){
			(
				// 设置当前语句
				statements.statement = new ArgumentsStatement(statements)
			)
			// 设置当前表达式
			.expression = new ArgumentsExpression(context);
		}
	});
...
this.RestTag = function(SpreadTag){
	/**
	 * 参数省略符标签
	 * @param {Number} _type - 标签类型
	 */
...
this.ArgumentNameTag = function(ArgumentExpression, ArgumentNameStatement){
	/**
	 * 参数名标签
	 * @param {Number} _type - 标签类型
	 */
	...
	ArgumentNameTag.props({
		 ...
		 */
		collectTo: function(parser, context, statements){
			var collection = statements.statement.expression.collection, content = context.content;
			...
			if(
				...
				// 报错
				parser.error(context, "Duplicate parameter name not allowed in this context");
				return;
			}
		...
		visitor: function(parser, context, statement, statements){
			...
			(
				// 设置当前语句
				statements.statement = new ArgumentNameStatement(statements)
			)
			// 设置当前表达式
	...
	this.ArgumentExpression,
	this.ArgumentNameStatement
);

this.RestArgumentNameTag = function(ArgumentNameTag, RestArgumentExpression){
	/**
	 * 省略参数名标签
	 * @param {Number} _type - 标签类型
	 */
	...
	RestArgumentNameTag.props({
		 ...
		 */
		visitor: function(parser, context, statement){
			var inner = statement.expression.inner;

			// 添加省略参数表达式
			inner.add(
				new RestArgumentExpression(context, inner.length)
			);
		}
	});
...
this.OpenFunctionBodyTag = function(OpenBlockTag, FunctionBodyStatements){
	/**
	 * 起始函数主体标签
	 * @param {Number} _type - 标签类型
	 */
...
this.CloseFunctionBodyTag = function(CloseBlockTag){
	/**
	 * 结束函数主体标签
	 * @param {Number} _type - 标签类型
	 */
...
this.CloseFunctionDeclarationBodyTag = function(CloseFunctionBodyTag, visitor){
	/**
	 * 结束函数声明主体标签
	 * @param {Number} _type - 标签类型
	 */
	function CloseFunctionDeclarationBodyTag(_type){
		CloseFunctionBodyTag.call(this, _type);
	};
	CloseFunctionDeclarationBodyTag = new Rexjs(CloseFunctionDeclarationBodyTag, CloseFunctionBodyTag);

	CloseFunctionDeclarationBodyTag.props({
		/**
		 * 获取此标签接下来所需匹配的标签列表
		 * @param {TagsMap} tagsMap - 标签集合映射
		 */
	...
	return CloseFunctionDeclarationBodyTag;
}(
	this.CloseFunctionBodyTag,
	this.CloseFunctionBodyTag.prototype.visitor
);
...
closeFunctionDeclarationBodyTag = new this.CloseFunctionDeclarationBodyTag();
	...
	// config
	new SyntaxConfig("defaultArgument", "restArgument"),
...
this.ObjectShorthandMethodStatement = function(){
	/**
	 * 对象简写方法语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
...
this.OpenShorthandMethodTag = function(ShorthandMethodHeadExpression, FunctionExpression, ObjectShorthandMethodStatement, visitor){
	/**
	 * 对象起始简写方法标签
	 * @param {Number} _type - 标签类型
	 */
	...
	OpenShorthandMethodTag.props({
		...
		visitor: function(parser, context, statement, statements){
			...
			.expression = new FunctionExpression(
				new ShorthandMethodHeadExpression()
			);
...
this.VarStatement = function(){
	...
	VarStatement.props({
		 ...
		 */
			...
			// 添加表达式
			expression.list.add(this.expression);
			// 如果是逗号，返回指定的分隔符，否则返回 null
			return context.content === "," ? vdsTag : null;
		},
		/**
		 * 尝试处理异常
		 * @param {SyntaxParser} parser - 语法解析器
...
this.VDSTag = function(CommaTag, VarStatement){
	/**
	 * var 语句变量声明分隔符标签(Var Declaration Sparator Tag)
	 * @param {Number} _type - 标签类型
	 */
...
// let 语句相关
void function(VarExpression, VarStatement, ClosureVariableTag, ldsTag){

this.LetStatement = function(catchMethod, tryMethod){
	/**
	 * let 语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
	function LetStatement(statements){
		VarStatement.call(this, statements);
	};
	LetStatement = new Rexjs(LetStatement, VarStatement);
	...
	LetStatement.props({
		 ...
		 */
			...
			// 如果父类方法有返回，则覆盖返回标签
			return catchMethod.call(this, parser, context) && ldsTag;
		},
		/**
		 * 尝试处理异常
		 * @param {SyntaxParser} parser - 语法解析器
		...
		try: function(parser, context){
			// 如果父类方法有返回，则覆盖返回标签
			return tryMethod.call(this, parser, context) && ldsTag;
		}
	});
...
this.LetTag = function(VarTag, LetStatement, config){
	/**
	 * let 关键字标签
	 * @param {Number} _type - 标签类型
	 */
	...
	LetTag.props({
		...
		visitor: function(parser, context, statement, statements){
			...
			statement.expression = new VarExpression(context);
			// 设置当前语句
			statements.statement = new LetStatement(statements);
		}
	});
...
this.LDSTag = function(VDSTag, LetStatement){
	/**
	 * let 语句变量声明分隔符标签(Let Declaration Sparator Tag)
	 * @param {Number} _type - 标签类型
	 */
...
// const 语句相关
void function(VarExpression, AssignableExpression, LetStatement, LocalVariableTag, cdsTag){

this.ConstStatement = function(ERROR, catchMethod, tryMethod){
	/**
	 * const 语句
	 * @param {Statements} statements - 该语句将要所处的语句块
	 */
	function ConstStatement(statements){
		LetStatement.call(this, statements);
	};
	ConstStatement = new Rexjs(ConstStatement, LetStatement);
	...
	ConstStatement.props({
		 ...
		 */
			...
			if(
				...
				// 由于没有赋值操作，则报错
				parser.error(this.expression.context, ERROR);
				// 返回分隔符标签
				return null;
			}

			return catchMethod.call(this, parser, context) && cdsTag;
		},
		/**
		 * 尝试处理异常
		 * @param {SyntaxParser} parser - 语法解析器
		...
		try: function(parser, context){
			// 如果父类方法有返回分隔符标签
			if(
				tryMethod.call(this, parser, context)
			){
				// 如果表达式是可赋值表达式
				if(
					this.expression instanceof AssignableExpression
	...
	LetStatement.prototype.catch,
	LetStatement.prototype.try
);
...
this.CDSTag = function(LDSTag, ConstStatement){
	/**
	 * const 语句变量声明分隔符标签(Const Declaration Sparator Tag)
	 * @param {Number} _type - 标签类型
	 */
...
this.ForExpression = function(config, compileOf){
	/**
	 * for 表达式
	 * @param {Context} context - 语法标签上下文
	 */
	...
	function(condition, body, contentBuilder, builder, variable){
		...
		// 追加对象值的初始化
		contentBuilder.appendString(
			builder.complete() + "=" + variable + ".next().value;"
		);
...
this.CatchStatement = function(){
	...
	CatchStatement.props({
		 ...
		 */
			...
			return context.content === "finally" ? finallyTag : null;
		}
	});
...
this.SwitchBodyStatements = function(BlockBodyStatements, SwitchBodyStatement, ECMAScriptVariableCollections, VariableCollection){
	/**
	 * switch 主体语句块
	 * @param {ECMAScriptVariableCollections} prevCollections - 上一个变量名收集器
	 */
	...
	SwitchBodyStatements.props({
		 ...
		 * @param {ECMAScriptVariableCollections} prevCollections - 上一个变量名收集器
		 */
		initCollections: function(prevCollections){
			var collections = new ECMAScriptVariableCollections(prevCollections.index);

			collections.blacklist = new VariableCollection();
			collections.const = new VariableCollection();
			collections.declaration = new VariableCollection();
			collections.rex = prevCollections.rex;
...
// case 表达式相关
void function(caseTag, defaultCaseTag, caseValueSeparatorTag){
...
this.CaseBodyStatements = function(BlockBodyStatements, CaseBodyStatement, ECMAScriptVariableCollections, VariableCollection){
	/**
	 * case 语句块
	 * @param {ECMAScriptVariableCollections} prevCollections - 上一个变量名收集器
	 */
...
this.ArgumentNameTags = function(ArgumentNameTag){
	/**
	 * 参数名标签列表
	 */
	function ArgumentNameTags(){
	...
	ArgumentNameTags = new Rexjs(ArgumentNameTags, IllegalTags);

	ArgumentNameTags.props({
		id: "argumentNameTags"
	});
	
	return ArgumentNameTags;
}(
	this.ArgumentNameTag
);

this.ArgumentSeparatorContextTags = function(ArgumentNameTags, RestTag){
	/**
	 * 参数分隔符上下文标签列表
	 */
	function ArgumentSeparatorContextTags(){
		ArgumentNameTags.call(this);
		
		this.register(
			new RestTag()
		);
	};
	ArgumentSeparatorContextTags = new Rexjs(ArgumentSeparatorContextTags, ArgumentNameTags);
...
this.FunctionDeclarationContextTags = function(FunctionContextTags, OpenArgumentsTag){
	/**
	 * 函数声明上下文标签列表
	 */
...
this.OpenArgumentsContextTags = function(ArgumentNameTag, RestTag){
	/**
	 * 参数起始上下文标签列表
	 */
	...
	OpenArgumentsContextTags = new Rexjs(OpenArgumentsContextTags, ECMAScriptTags);
...
this.OpenSwitchBodyContextTags = function(CaseTag, DefaultCaseTag, CloseBlockTag){
	/**
	 * switch 语句块起始上下文标签列表
	 */
...
this.ECMAScriptParser = function(ECMAScriptTagsMap, ECMAScriptStatements, tagsMap, parse){
	/**
	 * ECMAScript 语法解析器
	 */
