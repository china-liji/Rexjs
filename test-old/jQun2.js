...
// jQun的定义
new function(window, defineProperty){

this.jQun = function(create){
	function jQun(constructor, _SuperClass){
		///	<summary>
		///	生成一个继承指定父类的新生类。
		///	</summary>
		///	<param name="constructor" type="Function">新生类的构造函数。</param>
		///	<param name="_SuperClass" type="jQun">需要继承的父类。</param>
		var callee = arguments.callee;

		if(
			!callee.isInstanceOf(this, callee)
		){
			return new callee.HTMLElementList(arguments[0], arguments[1]);
		}

		constructor.toString = callee.toString;

		return constructor.prototype = create(
...
defineProperty(window, "jQun", { value : this.jQun });
}(
	window,
...
// 基本方法和属性的定义
new function(jQun, Object, Array, getOwnPropertyNames, getOwnPropertyDescriptor){
...
this.define = function(defineProperties, set, create){
...
this.prototype = function(prototype, forEach, define, defineProperties, getPrototypeOf){
	defineProperties(
		prototype,
		{
			...
			isSubOf : function(SuperClass){
				///	<summary>
				///	判断该类是否是指定类的子孙类。
				///	</summary>
				///	<param name="SuperClass" type="jQun, Function">指定的类，或指定类的构造函数。</param>
				return this instanceof SuperClass.constructor;
			},
	...
	/*
		...
		而新版本的chrome等浏览器，如果对象不是继承Object对象，那么则无__proto__属性。
		
		(2013.08.20)目前有些浏览器不支持，如：手机QQ浏览器，手机百度浏览器；
		(2015.01.06)nodejs也不支持。
	*/
...
// 面向对象的基础
new function(jQun, ARG_LIST_REGX, ARG_REGX, emptyArray, defineProperties, forEach, freeze){

// 构造函数与继承：类、静态类、接口
(function(Function, getOwnPropertyDescriptor, getArgumentNames, getArgumentsString){

this.Class = function(getSuperClass, AnonymousClass, getConstructor){
	function Class(_constructor, _SuperClass){
		...
		return new jQun(
			getConstructor(
				constructor,
				constructor.name || AnonymousClass.name,
				getArgumentNames(constructor),
				getArgumentNames(
					_SuperClass ? _SuperClass.constructor.source : null
				)
			),
			_SuperClass || this.getOwnClass()
		);
		...
		return defineProperties(
			new Function([
				"return function " + name + " (){",
					"var callee = arguments.callee, Super = this.getSuperClass.call(callee.prototype);",
...
this.StaticClass = function(AnonymousStaticClass, getConstructor){
	function StaticClass(_constructor, _props, _descriptor){
		...
		CreatedStaticClass = new jQun(
			getConstructor(
				constructor.name || AnonymousStaticClass.name
			),
			this.getOwnClass()
		);
...
this.Interface = function(Class, isInstanceOf, getConstructor, getName){
	function Interface(propertyNames, _Super){
		...
		var CreatedInterface, Super = _Super || Class.prototype;

		CreatedInterface = new jQun(
			getConstructor(
				propertyNames,
				getArgumentNames(Super.constructor.source)
			),
			Super
		);
	...
	function(propertyNames, argumentNames){
		...
		return defineProperties(
			...
			{
				source : new Function([
					"return function Interface(" + argumentNames.join(", ") + "){};"
				].join(""))()
			}
		);
	},
	...
	function(parentArgumentNames, argumentNames){
		if(
			(parentArgumentNames.length || argumentNames.length) === 0
		){
			return "";
		}
...
this.AttributesCollection = function(){
	...
	AttributesCollection = new Class(AttributesCollection, ElementPropertyCollection.prototype);
...
this.CSSPropertyCollection = function(isNaN, hasOwnProperty){
	...
	CSSPropertyCollection = new Class(CSSPropertyCollection, ElementPropertyCollection.prototype);
...
this.ChildrenCollection = function(){
	...
	ChildrenCollection = new Class(ChildrenCollection, ElementPropertyCollection.prototype);
...
this.ClassListCollection = function(){
	...
	ClassListCollection = new Class(ClassListCollection, ElementPropertyCollection.prototype);
...
this.DatasetCollection = function(AttributesCollection, PREFIX){
	...
	DatasetCollection = new Class(DatasetCollection, AttributesCollection.prototype);
...
this.EventTargetList = function(toArray){
	...
	EventTargetList = new Class(EventTargetList, List.prototype);
...
this.NodeList = function(EventTargetList, DOCUMENT_POSITION_SAME, DOCUMENT_POSITION_CONTAINED_BY, DOCUMENT_POSITION_FOLLOWING_AND_CONTAINED_BY){
	...
	NodeList = new Class(NodeList, EventTargetList.prototype);
...
this.ElementList = function(NodeList, Node, ID, selectorText, document){
	...
	ElementList = new Class(ElementList, NodeList.prototype);
...
this.HTMLElementList = function(ElementList, getCSSNumberValue, getComputedStyle, addProperty){
	...
	HTMLElementList = new Class(HTMLElementList, ElementList.prototype);
...
this.LocalCache = function(Cache){
	...
	LocalCache = new Class(LocalCache, Cache.prototype);
	
	LocalCache.props({
		type : "local"
	});

	return LocalCache.constructor;
}(
	this.Cache
);
...
this.SessionCache = function(Cache){
	...
	SessionCache = new Class(SessionCache, Cache.prototype);
...
this.RequestConnection = function(ConnectionMethods, ResponseTypes, implements, getEncodedParams){
...
this.PortList = function(List){
	...
	PortList = new Class(PortList, List.prototype);
...
this.Port = function(Socket, WINDOW_OBJECT_STRING, undefined, messageEvent, dispatch, set, toArray){
	...
	Port = new Class(Port, EventTargetList.prototype);
...
this.CrossWindowPort = function(Port){
	...
	CrossWindowPort = new Class(CrossWindowPort, Port.prototype);
	...
	return CrossWindowPort.constructor;
}(
	this.Port
);
...
this.TransferPort = function(CrossWindowPort, Socket){
	...
	TransferPort = new Class(TransferPort, CrossWindowPort.prototype);

	return TransferPort.constructor;
}(
